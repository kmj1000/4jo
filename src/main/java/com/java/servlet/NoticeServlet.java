...
public class NoticeServlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if( SESS_AUTH ) {
         ...
         int totalCount = service.getCountAllNotice();
         ...
         // allboard 가져와야 함.
         List<NoticeVO> noticeList =service.getAllNoticeByPage(pageMaker);//getAllBoardByPage
         //List<BoardVO> boardList =service.getAllBoard();//getAllBoardByPage
         System.out.println(noticeList);
         request.setAttribute("noticeList", noticeList);
         
         request.setAttribute("pageMaker", pageMaker);
         RequestDispatcher dispatcher = request.getRequestDispatcher( "/view/notice.jsp");
