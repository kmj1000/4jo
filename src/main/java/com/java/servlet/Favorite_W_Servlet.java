...
public class Favorite_W_Servlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if( SESS_AUTH ) {
         ...
         int totalCount = service.getCountWithPetBoard();
         ...
         // allboard 가져와야 함.
         List<Favorite_W_VO> boardList =service.getWithPetBoardByPage(pageMaker);//getAllBoardByPage
         //List<BoardVO> boardList =service.getAllBoard();//getAllBoardByPage
         System.out.println(boardList);
         request.setAttribute("boardList", boardList);
         
         request.setAttribute("pageMaker", pageMaker);
         RequestDispatcher dispatcher = request.getRequestDispatcher( "/view/favoritew.jsp");
